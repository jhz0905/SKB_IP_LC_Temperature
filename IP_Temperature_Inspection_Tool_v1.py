...
### Part. ASR9912 ###    
    ...
    if hw_type == "ASR 9912" or hw_type == "ASR 9006" or hw_type == "ASR 9922":
        for tem_start1 in range(len(log_line)):
            tem_pattern1 = r'^\w\w/\d/\w+\d/\w\w\w\d:.*#show environment temperatures$'
        ...
        for tem_start2 in range(tem_line1+1, len(log_line)):
            tem_pattern2 = r'^\w\w/\d/\w+\d/\w\w\w\d:.*#(admin|show| |)'
        ...
        for pid_start in range(len(log_line)):
            pid_pattern = r'^\w\w/\d/\w+\d/\w\w\w\d:.*#admin show (plat|platform)$'
        ...
        for pid_stop in range(len(log_line)):
            pid_pattern = r'^\w\w/\d/\w+\d/\w\w\w\d:.*#show environment temperatures$'
        ...
        for search_pid in range(pid_line1, pid_line2):
            ...
            if pid_result != None:
                ...
                pid_pattern = r'\w+-\w+(-\w+){0,2}'
                pid_name = re.search(pid_pattern, pid_result2) 
                pid_name = pid_name.group()
                if pid_name == "A9K-MPA-20X10GE" or pid_name == "A9K-MPA-20X1GE":
                    pass
                else:
...
### Used LC Slot Result ### 
    ...
    wb = load_workbook("%s\SKB IP망 10월 정기점검.xlsx" %nowdir)
    ws = wb["SKB IP망 23년 10월 정기점검_(Cisco)"]
    for excel_row in range(2,124):
        if ws["D%s" %excel_row].value == hostname:
            if hw_type == "WS-C6509-E" or hw_type == "CISCO7609-S" or hw_type == "WS-C6509" or hw_type == "WS-C6509-NEB-A":
                vaild_slot = 7 - len(lc_slot)
                ws["E%s" %excel_row] = high_lcnum
                ws["F%s" %excel_row] = compare_result
                ws["G%s" %excel_row] = lc_threshold
                ws["H%s" %excel_row] = used_lc_result
                ws["I%s" %excel_row] = vaild_slot
                #ws["J%s" %excel_row] = high_lcpid
            else:
                ws["E%s" %excel_row] = high_lcnum
                ws["F%s" %excel_row] = compare_result
                ws["H%s" %excel_row] = used_lc_result
                if hostname == "SLKAS_AR1" or hostname == "SLKAS_AR2":
                    vaild_slot = 20 - len(lc_slot)
                    ws["I%s" %excel_row] = vaild_slot
                    #print(lc_slot)
                else:
                    vaild_slot = 10 - len(lc_slot)
                    ws["I%s" %excel_row] = vaild_slot
                #ws["J%s" %excel_row] = high_lcpid
                if high_lcpid == "A9K-24x10GE-TR" or high_lcpid == "A9K-40GE-TR" or high_lcpid == "A99-12x100GE" or high_lcpid == "A9K-MOD400-TR":
                    ws["G%s" %excel_row] = thnf
                    ...
                    ws["G%s" %excel_row] = thes
                elif high_lcpid == "A9K-24X10GE-1G-TR" or high_lcpid == "A9K-48X10GE-1G-TR":
                    ws["G%s" %excel_row] = thes
        else:
            pass
    print("\n")
    wb.save("%s\SKB IP망 10월 정기점검.xlsx" %nowdir)
